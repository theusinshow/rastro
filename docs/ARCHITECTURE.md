...
| [0022](./decisions/0022-um-teste-que-abre-o-produto.md) | Um teste que abre o produto | Entra `@playwright/test`: `npm run smoke` confere num navegador o que função pura não alcança — ponteiro chegando ao mapa, camadas sobrevivendo à troca de tema, câmera entrando plana. Nasceu de três defeitos que passavam pelos 443 testes. Roda à mão, nunca em CI, e caso novo só entra depois de ter falhado na presença do defeito. |
| [0021](./decisions/0021-o-primeiro-minuto-depois-de-entrar.md) | O primeiro minuto depois de entrar | Quem entra sem ponto de partida cai na tela que pergunta por ele — uma vez, e com saída visível. O passeio da câmera passa a rodar também no Explorar, com perfil **plano e ao norte**, e **termina no primeiro gesto**, para sempre: o mapa ali é ferramenta. Revelou que `arrival.ts` tinha deixado de ser emitido, e o app abria com a câmera torta. |
