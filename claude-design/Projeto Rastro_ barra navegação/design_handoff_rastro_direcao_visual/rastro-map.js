(function () {
  const C = {
    visited: '#93a86e', wanted: '#e5a338', unvisited: '#9b9082',
    hollow: '#191817', bone: '#ede6db', accent: '#e5a338', void: '#0e0d0c'
  };
  ...
  function pinHtml(p, selected) {
    ...
    const ring = p.fav
      ? `<circle cx="14" cy="14" r="12.5" fill="none" stroke="${col}" stroke-width="1.25" opacity=".55"/>`
      : '';
    const dot = p.photo
      ? `<circle cx="14" cy="14" r="2.1" fill="${p.status === 'unvisited' ? col : C.void}"/>`
      : '';
    const sel = selected
  ...
  class RastroMap extends HTMLElement {
    constructor() { super(); this._data = null; this._markers = []; }

    set data(v) {
      const next = typeof v === 'string' ? JSON.parse(v) : v;
      this._data = next;
    ...
    async connectedCallback() {
      ...
      this._booted = true;
      this.style.cssText = 'position:absolute;inset:0;display:block;background:#0d0d0c';
      ...
      this._map = L.map(host, {
        ...
        center: d.center || [-27.9, -49.3], zoom: d.zoom || 8,
        preferCanvas: true
      });
    ...
    _sync() {
      ...
      if (d.route && d.route.length > 1) {
        ...
        L.polyline(d.route, { color: C.void, weight: 2, opacity: 1, dashArray: '9 11', lineCap: 'butt' }).addTo(this._routeLayer);
      }
  ...
  if (!customElements.get('rastro-map')) customElements.define('rastro-map', RastroMap);
})();
